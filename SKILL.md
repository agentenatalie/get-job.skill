...
name: get-job
description: |
  实习.skill / get-job.skill：从调研岗位、改简历到拿 offer 的全流程求职 skill。
  三段链路：① 用 WebSearch 调研目标岗位到底要什么 → ② 把简历按岗位反向定位、把不对口的经历翻译成岗位语言 → ③ 按轮次拆面试 + 简历逐 bullet 深挖 + 表达训练 + 证据化审计拆雷；交付前必须跑质量闸门和覆盖矩阵。
