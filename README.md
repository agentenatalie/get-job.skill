...
| **③ 准备面试** | 简历每个 bullet 逐条深挖 + 证据化审计拆雷 + 风险地图 + 逐题话术，让你被追问也不崩 | `面试准备.md`   |
...
再往后，它会基于改后的简历反推岗位、生成一份**逐 bullet + 逐题面试准备**（节选）：

<div align="center">
<img src="assets/interview-prep.png" alt="面试准备示例：逐 bullet 深挖、每条 JD 怎么答、自我介绍模板、核心项目怎么讲" width="78%" />
</div>
...
3. **准备面试** — 基于改后简历反推考点，把每个 bullet 的追问、细节、证据和兜底逐条准备好，再写自我介绍和题库话术 → `面试准备.md`

中间会有关键确认点（简历定稿前、逐 bullet 深挖表/面试话术前），你过一眼对不对再继续。简历已经投出去改不了，就直接说"按我简历准备面试"，跳过第 2 步。
...
├── 改后简历.md      # ② 改写后的简历成品
└── 面试准备.md      # ③ 逐 bullet 深挖 + 自我介绍 + 风险地图 + 逐题话术
```
...
3. **逐 bullet 面试准备**——面试官会顺着简历追问两三层，提前把每个 bullet 的背景、角色、动作、数据口径、二三层追问和兜底都准备好；能讲清的最大化，讲不清的模糊化，用户无法确认的降级或删除。
